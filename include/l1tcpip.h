...
typedef struct
{
  ...
  int     tcpstat;                 /* Connect Statistik.                      */
  int     max_tcpstat;             /* Max. Stat-Eintraege.                     */
} T_INTERFACE;
...
typedef struct TCPIP               /* TCPIP-User/Link                         */
{
  ...
  int           datetime;          /* Login Datum & Zeit eintragen.           */
#ifdef L1HTTPD
...
struct tcpstat_table_entry {
   char callsign[L2IDLEN];                           /* the callsign and ssid */
   unsigned long ip_addr;                                   /* the IP address */
   char hostname[HNLEN];
   char httpd_content[47];
   char  timeset[17 + 1];                        /* Login-Zeit/Date. */
   UWORD          Interface;         /* Interface Telnet/Httpd/IPConvers/IRC. */
...
extern void    ShowTCP(struct tcpstat_table_entry, MBHEAD*, int);
extern int     routeTCP_add(char*, unsigned short);
extern BOOLEAN CheckCall(char *, WORD);
