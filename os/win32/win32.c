...
sigsegv(int signo)
{
  ...
  /* Debug-Meldung in das Konsolen-Fenster schreiben. */
  printf("\n*********************************************************************\n");
  printf("*                                                                   *\n");
  printf("* TheNetNode verursachte einen unbekannten Systemfehler!!!          *\n");
  printf("* Letzte Funktion war: %s\n", lastfunc);
  printf("*                                                                   *\n");
  printf("*********************************************************************\n");

  fprintf (Debug_fp, "Letzte Funktion war %s\r", lastfunc);
  fprintf (Debug_fp, "Vorhergehende Funktion war %s\r", lastfunk);
  Break();
  fclose(Debug_fp);
...
/************************************************************************/
void
sigstime(char *funktion)
{
  signal(SIGSEGV, SIG_IGN);
  /* Debug-Meldung in das Konsolen-Fenster schreiben. */
  printf("\n*********************************************************************\n");
  printf("*                                                                   *\n");
  printf("* TheNetNode wird vorzeitig beendet, weil eine Schleifendifferenz   *\n");
  printf("* von mehr als 25sekunden ueberschritten wurde!!!                   *\n");
  printf("*                                                                   *\n");
  printf("*********************************************************************\n");
  printf("* Letzte Funktion war: %s.                                           \n", funktion);
  printf("*********************************************************************\n");

  /* Debug-Meldung in debug.log schreiben! */
  fprintf(Debug_fp, "*********************************************************************\r\n");
  fprintf(Debug_fp, "*                                                                   *\r\n");
  fprintf(Debug_fp, "* TheNetNode wird vorzeitig beendet, weil eine Schleifendifferenz   *\r\n");
  fprintf(Debug_fp, "* von mehr als 25sekunden ueberschritten wurde!!!                   *\r\n");
  fprintf(Debug_fp, "*                                                                   *\r\n");
  fprintf(Debug_fp, "*********************************************************************\r\n");
  fprintf(Debug_fp, "* Letzte Funktion war: %s.                                             \r\n", funktion);
  fprintf(Debug_fp, "*********************************************************************\r\n");
...
void S_Ende(char *F_Name)
{
  time_t t;
  /* Funktion durchgelaufen, Zeit neu setzen.          */
  time(&debug_oldtime);
  time(&t);

  /* Berechnen ob der durchlauf zulange gedauert hat. */
  if (debug_newtime < debug_oldtime)
  {
    fp_schleifencheck = xfopen("debug2.log","at");                 /* File oeffnen     */

    if (fp_schleifencheck != NULL)
	{
      fprintf(fp_schleifencheck, "%s%s: Sleep %ld sekunden.\n", ctime(&t), F_Name, debug_oldtime - debug_newtime);
      fclose(fp_schleifencheck);
	}
  }
}
#endif
