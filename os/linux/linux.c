  ...
  if (access(file_name, X_OK) == 0)     /* nur ausfuehrbares Programm   */
   {
    ...
    execl(file_name, file_name, NULL);  /* Programm starten             */
...
sigsegv(int signo)
{
  ...
  fprintf (Debug_fp, "Vorhergehende Funktion war %s\n", lastfunk);
...
void S_Ende(char *F_Name)
{
  ...
  {
    fp_schleifencheck = xfopen("debug2.log","wb");                 /* File oeffnen     */
