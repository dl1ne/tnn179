...
BOOLEAN axipx_l1init(int l2port)
{
  ...
  {
#ifdef SPEECH
    xprintf(speech_message(331), strerror(errno));
#else
    xprintf("cannot create socket for IPX : %s\n", strerror(errno));
#endif
    return(FALSE);
  }
  ...
  {
#ifdef SPEECH
    xprintf(speech_message(332), strerror(errno));
#else
    xprintf("cannot bind IPX address: %s\n", strerror(errno));
#endif
    close(l1pp->kisslink);
    l1pp->kisslink = -1;
    return(FALSE);
  ...
  {
#ifdef SPEECH
      xprintf(speech_message(333));
#else
      xprintf("Error: can't set non-blocking I/O on IPX socket");
#endif
      close(l1pp->kisslink);
      l1pp->kisslink = -1;
      return(FALSE);
