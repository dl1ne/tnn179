...
printf("DEBUG: ostcpip.c - OsStackReadIndicationsSock()\n");
#endif
  ...
  while(1)                               /* Alle Zeichen von Socket einlesen. */
  {
    ...
    switch(tcppoi->Interface)            /* Interface.                        */
    {
      ...
      case KISS_IPCONV :
#ifdef DEBUG_OSTCPIP_C
printf("DEBUG ostcpip.c - OsStackReadIndicationsSock - KISS_IPCONV\n");
#endif
      ...
      case KISS_IRC :
#ifdef DEBUG_OSTCPIP_C
printf("DEBUG: ostcpip.c - OsStackReadIndicationsSock() - KISS_IRC\n");
#endif
        /* Aktuelle Zeichen auswerten. */
        if ((ok = GetContensCVS(s)) == ERRORS)
        {
          /* Alle Zeichen verarbeitet, aber kein Return. */
#ifdef DEBUG_OSTCPIP_C
printf("DEBUG: ostcpip.c - OsStackReadIndicationsSock() - KISS_IRC - Zeichen verarbeitet, aber kein Return\n");
#endif
          break;
        }
        else
...
#ifdef DEBUG_OSTCPIP_C
printf("DEBUG: ostcpip.c - ServOS() - tcppoi->cmd: %s", tcppoi->cmd);
#endif

      for (i = 0; i < tcppoi->cmdlen; ++i)
        putchr(tcppoi->cmd[i], SRx->Data);                 /* Buffer fuellen. */

      /*tcppoi->cmd[strcspn(tcppoi->cmd, "\r\n")] = 0; */  /* DL1NE: Test LF entfernen */
  ...
  for (i = 0; i < MAXINTERFACE; i++, Interface++)
  {
    ...
    if (FD_ISSET(ifpp->OsSock, &rmask))         /* Interface-Socketvergleich. */
    {
      ...
      addrlen = sizeof peeraddr_in;
          ...
          save_fd = fcntl(ifpp->OsSock, F_GETFL);
          save_fd |= O_NONBLOCK;

          fcntl(ifpp->OsSock, F_SETFL, save_fd);

      if ((NewSock = accept(ifpp->OsSock, (struct sockaddr *) &peeraddr_in, &addrlen)) != EOF)
      {
        save_fd = fcntl(NewSock, F_GETFL);
            save_fd |= O_NONBLOCK;

            fcntl(NewSock, F_SETFL, save_fd);
