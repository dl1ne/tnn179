...
#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - putcvsu()\n");
#endif
...
{
#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - putstr(%s, mbp)\n", string);
#endif
  while (*string)
    putchr(*string++, mbp);
}
...
#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - valcal() - Checking Call: %s\n", call);
#endif
...
#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - valcal() - first\n");
#endif
  ...
  /* Sonderlocke, damit IGATE durchkommt */
  if (strncmp(call, "IGATE", 5) == 0)
    return(YES);

#ifndef DISABLECALLCHECK
  if (   ((actual - call) < 4) || zahl == 0 || zahl > 2 || (numpos == call)
      || (numpos == (actual - 1)) || ((numpos - call) >= 3)) {

#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - valcal() - CALLCHECK \n");
#endif
  ...
  if (i < 4) {
    /* ist es ungueltig. */
#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - valcal() - Zu kurz!\n");
#endif

    return(ERRORS); }
#endif /* DISABLECALLCHECK */
...
printf("DEBUG: l7utils.c - getlin()\n");
  ...
  if (userpo->convers) {
#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - getlin() - userpo->convers\n");
#endif
...
#ifdef DEBUG_L7UTILS_C
printf("DEBUG: l7utils.c - getlin() - after loop\n");
#endif
  mbp->mbbp = nextch;
  mbp->mbgc = getcou;
  return(found);
...
#ifdef DEBUG_MODUS
  lastfunc = "l7utils(intern_command)";
#endif /* DEBUG_MODUS */
...
void send_async_response(USRBLK *ublk, const char *title, const char *text) {
  ...
  LHEAD   mbhd;
#ifdef DEBUG_MODUS
  lastfunc = "l7utils(send_async_response)";
#endif /* DEBUG_MODUS */
