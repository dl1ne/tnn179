...
#ifdef SETPATH
static void     SYSPath_Init(void);                     /* System-Verzeichnisse anlegen. */
#endif /* SETPATH */
...
int main(int argc, char *argv[])
{
...
#ifdef DEBUG_MODUS
  Debug_fp = xfopen("debug.log","wb");                 /* File oeffnen     */
#endif
  ...
  *confpath = NUL;
...
#ifdef SETPATH                                                  /* Systemverzeichnisse anlegen. */
  SYSPath_Init();
#endif
  ...
  if (putenv(line) == -1)
    xprintf("*** WARNING: Enviroment full ***\n");
...
static void mainf(void)
{
  ...
  hputid(myid);
  hprintf(")\r"
          "Copyright by NORD><LINK e.V. (c)1998 - 2015\r"
...
{
#ifdef SPEECH
  xprintf(speech_message(280));
#else
  xprintf("Programm wird beendet!\n");
#endif
...
/***********************************************************/
/*                                                         */
/* Systemverzeichnisse anlegen.                            */
/*                                                         */
/***********************************************************/
#ifdef SETPATH

static void SYSPath_Init(void)
{
#ifdef __LINUX__
  mode_t mode = 0775;
#endif /* LINUX */
  ...
  if (xaccess(confpath, 0) != 0)                  /* Ist noch kein Haupt-Vezeichnis erstellt?  */
    mkdir(confpath, mode);                                     /* Haupt-Verzeichnis erstellen. */
#else
  if (xaccess(confpath, 0) != 0)                  /* Ist noch kein Haupt-Vezeichnis erstellt?  */
      mkdir(confpath);                                               /* Verzeichnis erstellen. */
#endif /* LINUX / GO32 */

#if defined(__LINUX__) || defined(__GO32__)
  if (xaccess(textcmdpath, 0) != 0)                     /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(textcmdpath, mode);                                        /* Verzeichnis erstellen. */
#else
  if (xaccess(textcmdpath, 0) != 0)                     /* Ist noch kein Vezeichnis erstellt?  */
      mkdir(textcmdpath);                                            /* Verzeichnis erstellen. */
#endif /* LINUX / GO32 */

#if defined(__LINUX__) || defined(__GO32__)
  if (xaccess(userexepath, 0) != 0)                     /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(userexepath, mode);                                        /* Verzeichnis erstellen. */
#else
  if (xaccess(userexepath, 0) != 0)                     /* Ist noch kein Vezeichnis erstellt?  */
      mkdir(userexepath);                                            /* Verzeichnis erstellen. */
#endif /* LINUX / GO32 */

#if defined(__LINUX__) || defined(__GO32__)
  if (xaccess(sysopexepath, 0) != 0)                    /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(sysopexepath, mode);                                       /* Verzeichnis erstellen. */
#else
  if (xaccess(sysopexepath, 0) != 0)                    /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(sysopexepath);                                             /* Verzeichnis erstellen. */
#endif /* LINUX / GO32 */

#if defined(__LINUX__) || defined(__GO32__)
  if (xaccess(msgpath, 0) != 0)                         /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(msgpath, mode);                                            /* Verzeichnis erstellen. */
#else
  if (xaccess(msgpath, 0) != 0)                         /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(msgpath);                                                  /* Verzeichnis erstellen. */
#endif /* LINUX / GO32 */

#ifdef PACSAT
#if defined(__LINUX__) || defined(__GO32__)
  if (xaccess(pacsatpath, 0) != 0)                      /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(pacsatpath, mode);                                         /* Verzeichnis erstellen. */
#else
  if (xaccess(pacsatpath, 0) != 0)                      /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(pacsatpath);                                               /* Verzeichnis erstellen. */
#endif /* LINUX / GO32 */
#endif /* PACSAT */

#ifdef SPEECH
#if defined(__LINUX__) || defined(__GO32__)
  if (xaccess(speechpath, 0) != 0)                      /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(speechpath, mode);                                         /* Verzeichnis erstellen. */
#else
  if (xaccess(speechpath, 0) != 0)                      /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(speechpath);                                               /* Verzeichnis erstellen. */
#endif /* LINUX / GO32 */
#endif /* SPEECH */

#ifdef AXIPR_HTML
#if defined(__LINUX__) || defined(__GO32__)
  if (xaccess(htmlpath, 0) != 0)                        /* Ist noch kein Vezeichnis erstellt?  */
  mkdir(htmlpath, mode);                                             /* Verzeichnis erstellen. */
#else
  if (xaccess(htmlpath, 0) != 0)                        /* Ist noch kein Vezeichnis erstellt?  */
    mkdir(htmlpath);                                                  /* Verzeichnis erstellen.*/
#endif /* LINUX / GO32 */
#endif /* AXIPR_HTML */
}
