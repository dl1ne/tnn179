...
#ifdef MOD_L1HTTPD_C_NEWMIMETYPES
enum { NONE, BIN, TXT, GIF, JPG, WAV, MP3, CSS, JS} h_mimetype;
#else /* MOD_L1HTTPD_C_NEWMIMETYPES */
enum { NONE, BIN, TXT, GIF, JPG, WAV, MP3 } h_mimetype;
#endif /* MOD_L1HTTPD_C_NEWMIMETYPES */
...
void ccphttpd(void)
{
  ...
  mbp = putals("HTTPD-Server:\r\r*** Connect-Statistik ***\rDatum-----------IP--------------Client-----------------------------------------\r");

  ifpp = &ifp[HTP_ID];

  /* Alle TCPIP-Aktivitaeten ausgeben */
  for (i = 0; i < ifpp->tcpstat; i++)
    ShowTCP(tcpstat_tbl[ifpp->Interface][i], mbp, KISS_HTTPD);

  putprintf(mbp, "\r*** HTTPD-Server Einstellungen ***\r");
...
#ifdef SPEECH
      xprintf(speech_message(339));
#else
      xprintf("form_referer str too long");
#endif
...
static void GetRequest(MBHEAD *data)
{
  ...
  if (locuri)
  {
    strncpy(h_uri, locuri, 200);
        ...
        if (tcppoi->id)
      /* Fuer unsere Statistik. */
          strncpy(tcpstat_tbl[ifpp->Interface][tcppoi->id].httpd_content, h_uri, 46);
  }
  ...
  if (stricmp(locmethod, "GET") == 0)
    ...
    h_status = 501;
#ifdef DEBUG_L1HTTPD_C
printf("DEBUG: l1httpd.c - before ext detecting: %s\n", h_uri);
#endif /* DEBUG_L1HTTPD_C */
  if ((ext = strchr(h_uri,'.')) == 0)
  {
    if(h_file[strlen(h_file)-1]=='/')
      h_file[strlen(h_file)-1]=0;
  }
#ifdef MOD_L1HTTPD_C_ALLOWDOTINCMD
  if (ext != NULL && stricmp(h_uri, "/cmd") == TRUE)
#else
  if (ext != NULL)
#endif /* MOD_L1HTTPD_C_ALLOWDOTINCMD */
  {
#ifdef DEBUG_L1HTTPD_C
printf("DEBUG: l1httpd.c - if (ext != NULL), ext = %s\n", ext);
#endif /* DEBUG_L1HTTPD_C */
    if (  (strstr(h_uri, "htm")
        ||(strstr(h_uri, "php"))) == 0)
    {
  ...
  if(locprotocol && stristr(locprotocol, "HTTP/") == locprotocol)
  {
    ...
    if(h_contentlength)
    {
      ...
      if (h_contentlength > 10000000L)
        xprintf("httpd::start_http content too long");
  ...
  {
#ifdef DEBUG_L1HTTPD_C
printf("DEBUG: l1httpd.c - if (ext != NULL)\n");
#endif /* DEBUG_L1HTTPD_C */
  ...
  {
#ifdef DEBUG_L1HTTPD_C
printf("DEBUG: l1httpd.c - if (stricmp(h_uri, /cmd) == 0)\n");
#endif /* DEBUG_L1HTTPD_C */
...
#ifdef MOD_L1HTTPD_C_NEWMIMETYPES
      case CSS: mimestr = MIME_CSS; break;
      case JS:  mimestr = MIME_JS ; break;
#endif /* MOD_L1HTTPD_C_NEWMIMETYPES */
...
#ifdef MOD_L1HTTPD_C_CMDLENGTH
        get_postarea("cmd=",cmd,220,0);
#else /* MOD_L1HTTPD_C_CMDLENGTH */
        get_postarea("cmd=",cmd,29,0);
#endif /* MOD_L1HTTPD_C_CMDLENGTH */
...
void SetParentPath(char *Path, char *ParentPath)
{
  ...
  loc = strrchr(TmpPath2, '/');

  if (loc == NULL)
    loc = strrchr(TmpPath2, FILE_SEP);
    ...
    else
    {
        ...
        ||(strcmp(h_uri, "/login") == 0))
      {
        ...
        if (SetFileOpen(("/index.html"), tbp) == TRUE)
          /* Externe HTML-Datei kann  nicht geoffnet werden. */
          PutHtmlError(tbp, "httpd/index.html");
        else
          /* Externe Datei laden. */
          tcppoi->status = TCP_URI;
      }
      /* evl. Kommando ausfuehren. */
        ...
        else
        {
          ...
          char          *ParentPath =Buffer2;
                  ...
                  strcpy(h_cfgfile, htmlpath);
                  datei++;
                  strcat(h_cfgfile, datei);

                  /* Pruefe ob es eine Datei oder Verzeichnis ist. */
          if ((fp = xfopen(h_cfgfile, "rt")) != NULL)
          {
                        /* Es ist eine Datei. */
            fclose(fp);
            /* Auch kein Befehl.   */
...
#ifdef MOD_L1HTTPD_C_LOCALHOSTISSYSOP
      if (strcmp(tcppoi->ip, "127.0.0.1") != 0)      /* Keine Pruefung fuer lokale Anmeldungen */
      {
#endif /* MOD_L1HTTPD_C_LOCALHOSTISSYSOP */
