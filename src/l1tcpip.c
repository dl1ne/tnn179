...
int tcpstat_tbl_top;               /* Anzahl aller TCP-User im Laufenbetrieb. */
...
void InitIFC(void)                     /* Interface Initialisierung           */
{
  ...
  {                                    /* Initialisieren.                     */
    ...
    ifpp->log       = 0;               /* Loglevel.                           */
        ifpp->tcpstat   = 1;               /* Connect Aktivitaet.                  */
        ifpp->max_tcpstat = TCPSTAT_TABLE_SIZE; /* Max. Stat-Eintraege.            */
                        ...
                        BOOLEAN   Mode)               /* Interner TCP-Stack   */
{
  ...
  tcppoi->datetime    = time(NULL);      /* Login Datum & Uhrzeit setzen.     */
  tcppoi->Upcall[0]   = 0;
  tcppoi->Downcall[0] = 0;
#ifdef L1HTTPD
...
#endif /* DEBUG_MODUS */
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - ReadIndicationsSockTCP()\n");
#endif
...
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - ReadIndicationsSockTCP() -> KISS_IPCONV -> ok\n");
#endif
          break; }
        else
...
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - ReadIndicationsSockTCP() -> KISS_IPCONV -> nak\n");
#endif
            /* Frame komplett. */
            return(TRUE);
          }
      ...
      case KISS_IRC :
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - ReadIndicationsSockTCP() - KISS_IRC - run GetContensCVS(s), s = %s\n", s);
#endif
        /* Aktuelle Zeichen auswerten. */
        if ((ok = GetContensCVS(s)) == ERRORS) {
          /* Alle Zeichen verarbeitet, aber kein Return. */
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - ReadIndicationsSockTCP() - KISS_IRC - ok\n");
#endif
          break; }
        else
          /* Alle Zeichen verarbeitet und Return? */
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - ReadIndicationsSockTCP() - KISS_IRC - f#2, %c\n", ok);
#endif
          if (ok == YES)
          {
            tcppoi->cmd[tcppoi->cmdlen] = 0;
...
int routeTCP_add(char* hostname, UWORD Interface)
{
  int    i, j = 0;
  ...
  for (i = 0; i < MAXINTERFACE; i++)
  {
    ...
    if (ifpp->tcpstat >= ifpp->max_tcpstat)
    {
     for (j = 0; j < ifpp->tcpstat; j++)
         {
           if (tcpstat_tbl[ifpp->Interface][j].Interface == Interface)
           {
             for (i = j; i < ifpp->tcpstat; ++i)
           tcpstat_tbl[ifpp->Interface][i] = tcpstat_tbl[ifpp->Interface][i + 1];

         tcpstat_tbl[ifpp->Interface][i].callsign[0]      = '\0';
             tcpstat_tbl[ifpp->Interface][i].hostname[0]      = '\0';
             tcpstat_tbl[ifpp->Interface][i].httpd_content[0] = '\0';
             tcpstat_tbl[ifpp->Interface][i].Interface        = 0;
             tcpstat_tbl[ifpp->Interface][i].timeset[0]       = '\0';

         strcpy(tcpstat_tbl[ifpp->Interface][i].timeset, Set_Time());
         strcpy(tcpstat_tbl[ifpp->Interface][i].hostname, hostname);

         tcpstat_tbl[ifpp->Interface][i].Interface = Interface;
   ...
   strcpy(tcpstat_tbl[ifpp->Interface][ifpp->tcpstat].timeset, Set_Time());
   strcpy(tcpstat_tbl[ifpp->Interface][ifpp->tcpstat].hostname, hostname);
   tcpstat_tbl[ifpp->Interface][ifpp->tcpstat].callsign[0]      = NUL;
   tcpstat_tbl[ifpp->Interface][ifpp->tcpstat].httpd_content[0] = NUL;

   tcpstat_tbl[ifpp->Interface][ifpp->tcpstat].Interface = Interface;
...
/*          gefunden.                          */
/***********************************************/
static int SearchCall(char  *call, BOOLEAN ssid)
{
  TCPIP   *tpoi;
  int      i     = FALSE;
  int      j     = FALSE;
...
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - SearchCall\n");
#endif
...
BOOLEAN CheckCall(char *call, WORD len)  /* Zeiger fuer erweitere Logindaten. */
{
  ...
  if (getcal(&len,&call,FALSE,logincall) == YES)        /* Rufzeichen pruefen */
  {
    if (valcal(logincall) == ERRORS)    /* Pruefe Rufzeichen auf Gueltigkeit. */
    {
      ...
      return(TRUE);                                  /* Rufzeichen ungueltig. */
        ...
        /************************************************************************/
        /* TCPSTAT - Connect Aktivitaet - CALL eintragen.                        */
        /************************************************************************/
        ifpp = SetInterface(tcppoi->Interface);

    if (ifpp->tcpstat != EOF)
      cpyid(tcpstat_tbl[ifpp->Interface][tcppoi->id].callsign, pcall); /* Call fuer TCPSTAT. */
...
#endif /* DEBUG_MODUS */
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - LoginTCP()\n");
#endif
...
#ifdef L1TELNET
          case KISS_TELNET :
                  return(LoginTELNET(tbp));

      break;
#endif

#ifdef MOD_L1IPCONV_C_TELNETLOGIN
#ifdef L1IPCONV
         case KISS_IPCONV:

#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - KISS_IPCONV -> Telnet Login\n");
#endif
                  return(LoginTELNET(tbp));

#endif /* L1IPCONV */
#endif /* MOD_L1IPCONV_C_TELNETLOGIN */

#ifdef L1HTTPD
          case KISS_HTTPD :
                  return(LoginHTTPD(tbp));
...
void RelinkTCP(MBHEAD *tbp)
{
  ...
  {
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - RelinkTCP() - Es gibt Daten, versuche weiterleitung...\n");
#endif
  ...
  {
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - RelinkTCP() - Keine Daten gefunden, verwerfe Anfrage!\n");
#endif
  }
}
       ...
       SRx  = SRxPrev)
  {
    ...
    switch(tcppoi->Interface)
    {
      ...
      default :
        ...
        {
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - TcpipRX() - User ist nicht angemeldet, verwerfe Daten und Sende Login!\n");
#endif
          if (LoginTCP(SRx->Data) == FALSE)                     /* User/Link anmelden.   */
            tcppoi->disflg |= 0x80;           /* Anmeldung war nicht erfolgreich. */
        }
        else
        {                                          /* User-Link ist angemeldet.*/
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - TcpipRX() - User angemeldet, Frame weiterleiten\n");
#endif
...
#ifdef DEBUG_L1TCPIP_C
printf("DEBUG: l1tcpip.c - TcpipSV\n");
#endif
  ...
  {                                     /* Zeiger auf das aktuelle Interface. */
    ...
    if (FD_ISSET_T(ifpp->ISock, &Rmask))
    {
      ...
      T_LOGL2(TRUE, "(RecvTCP):\n"
          ...
          tcppoi->id = routeTCP_add(ip, ifpp->Interface);
      continue;                                   /* zum naechsten Interface. */
...
#ifdef DEBUG_MODUS_SCHLEIFENCHECK
  S_Start();
#endif
    ...
    case KISS_TELNET:
      {
        ...
        strcpy(logfile, confpath);
                strcat(logfile, TELNETLOG);
        fp = fopen(logfile, "a+");                          /* Datei oeffnen. */
          break;
      }
#endif /* L1TELNET */
...
/*************************************************************************/
/* Einen Eintrag aus der TCP-Tabelle ausgeben                            */
/*************************************************************************/
void ShowTCP(struct tcpstat_table_entry rp, MBHEAD *bufpoi, int Mode)
{
  if (rp.Interface == Mode)
  {
    /* erst mal merken, wo wir sind */
    bufpoi->l4time = bufpoi->mbpc;

    /* IP-Adresse ausgeben */
    putprintf(bufpoi, "%s ", rp.timeset);
    ...
    putprintf(bufpoi, "%s", rp.hostname);
        ...
        /* Nur fuer HTTPD-Server */
    if (rp.Interface == KISS_HTTPD)
          /* Get Anfrage ausgeben. */
      putprintf(bufpoi, "%s", rp.httpd_content);
        else
        {
                if (rp.callsign[0] != NUL)
                        putcal(rp.callsign, bufpoi);
        }
