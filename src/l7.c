...
#ifdef MOD_L7_C_LOCALHOSTISSYSOP
/* Lokale Benutzer direkt zum Sysop machen */
  if(strcmp(tcppoi->ip, "127.0.0.1") == 0) local = TRUE;
#endif /* MOD_L7_C_LOCALHOSTISSYSOP */
  ...
  switch (opcod)                        /* je nach Opcode               */
  {
   ...
   if (p_uid != NO_UID)          /* hat der Link einen Partner?      */
      ...
      else
      {
        ...
        {                           /* neuer User                       */
          if (!new_user(FALSE, uid, local))         /* User abgelehnt?         */
          {
            disusr(uid);
            lnkpoi = oldlp;
            cirpoi = oldcp;
    ...
    case L2MFAILW :
      if (p_uid != NO_UID)          /* User hatte einen Partner         */
      {
        ...
        if ((userpo = p_ptcp->ublk) == NULL)   /* Partner nicht im CCP? */
        {
          ...
          {
            if (new_user(TRUE, p_uid, local))         /* dann erneut in CCP    */
            {
...
/*----------------------------------------------------------------------*/
BOOLEAN new_user(BOOLEAN check, UID uid, BOOLEAN local)
{
  PERMLINK *p;
  WORD     i;
  char     call[L2IDLEN];
