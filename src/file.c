...
BOOLEAN save_configuration(void)
{
  ...
  if ((fp = xfopen(file,"wt")) != NULL)      /* File oeffnen            */
  {
    ...
    fprintf(fp,";\n"
        ...
        strcpy(t_cfgfile, confpath);
        strcat(t_cfgfile, TEXTCMDPATH);
        strcpy(textcmdpath, t_cfgfile);
    ...
    fprintf(fp,";\n"
        ...
        strcpy(t_cfgfile, confpath);
        strcat(t_cfgfile, USEREXEPATH);
        strcpy(userexepath, t_cfgfile);
    ...
    strcpy(t_cfgfile, confpath);
        strcat(t_cfgfile, SYSEXEPATH);
        strcpy(sysopexepath, t_cfgfile);
...
#ifdef PACSAT
        strcpy(t_cfgfile, confpath);
        strcat(t_cfgfile, PACSATPATH);
        strcpy(pacsatpath, t_cfgfile);
...
#ifdef AXIPR_HTML
        strcpy(t_cfgfile, confpath);
        strcat(t_cfgfile, HTMLPATH);
        strcpy(htmlpath, t_cfgfile);

    fprintf(fp,";\n"
               "; Path fuer HTML (" HTMLPATH ")\n"
               "%s\n", htmlpath);
#endif

#ifdef SPEECH
    strcpy(t_cfgfile, confpath);
        strcat(t_cfgfile, SPEECHPATH);
        strcpy(speechpath, t_cfgfile);

    fprintf(fp,";\n"
               "; Path fuer SPEECH-File (" SPEECHPATH ")\n"
               "%s\n", speechpath);
#endif

        strcpy(t_cfgfile, confpath);
        strcat(t_cfgfile, MSGPATH);
        strcpy(msgpath, t_cfgfile);

    fprintf(fp,";\n"
               "; Path fuer MSG-File (" MSGPATH ")\n"
               "%s\n", msgpath);
...
BOOLEAN save_stat(void)
{
  ...
  if ((fp = xfopen(file,"wb")) != NULL)             /* File oeffnen     */
  {
    ...
    if (ret == FALSE)
        xprintf("*** %s%s.STA write error ***\n",confpath, cfgfile);
    fclose(fp);                 /* File schliessen                      */
    return(ret);
  }
  xprintf("*** %s%s.STA open error ***\n",confpath, cfgfile);
  return(FALSE);
}
...
#ifdef HTMLPATHFIX
    strcpy(htmlpath, textpath);
    strcat(htmlpath, "httpd/");
#else
#ifdef AXIPR_HTML                                   /* HTTPD-Server-Path */
    fgetpath(htmlpath, fp);
#endif
#endif

#ifdef SPEECH                                       /* Speech-Path      */
    fgetpath(speechpath, fp);
#endif

#ifdef MSGPATHFIX
    fgetpath(msgpath, fp);                          /* MSG-Path         */
#endif /* MSGPATHFIX */
